...
class WavOpen:
    ...
    def get_all_frames(self):
        ...
        if rawdata:
            signal = wave.struct.unpack('%ih' % self.num_frames *
                                        self.num_channels,
                                        rawdata)  # transform to signal
            del rawdata
    ...
    def next(self):
        ...
        if num_to_read > 0:
            ...
            self.sig[:] = 0
            if rawdata:
                signal = wave.struct.unpack('%ih' %
                                            (num_to_read * self.num_channels),
                                            rawdata)  # transform to signal
                ...
                raise Exception(
                    "Could not read sufficient sound data at index %d" %
                    self.index)
        else:
            raise StopIteration
...
def _wav_write(signal, wav_name, sample_rate):
    ...
    if HAVE_AUDIOLAB:
        ...
        for i in range(signal.shape[1]):
            b_signal += wave.struct.pack('h', int(
                32767 * signal[0, i]))  # transform to C-style binary string
            if signal.shape[0] > 1:
                b_signal += wave.struct.pack('h', int(
                    32767 * signal[1, i]))  # transform to C-style binary string
...
if os.name == 'posix':
    ...
    def play_snd(data, sample_rate=44100):
        ...
        if m > 1.0: data /= m
        _wav_write(data, AUDIO_TMP_FILE, sample_rate)
        winsound.PlaySound(AUDIO_TMP_FILE, winsound.SND_FILENAME |
                           winsound.SND_ASYNC)
