...
class TuningSystem():
    ...
    def __init__(self):
        pitch_class_sharp = ['U', 'm2', 'M2', 'm3', 'M3', 'P4', 'TT', 'P5',
                             'm6', 'M6', 'm7', 'M7', 'P8']
        pitch_class_flat = ['M7', 'm7', 'M6', 'm6', 'P5', 'TT', 'P4', 'M3',
                            'm3', 'P2', 'm2', 'U']
    ...
    def equal_temperament(self, frame_interval=2, num_steps=12):
        ...
        self.pitch_ratios_eq = self.generalized_equal_temperament(
            frame_interval, num_steps)
    ...
    def Bohlen_Pierce(self, frame_interval=3, num_steps=13):
        ...
        """
        self.pitch_ratios_bp = self.generalized_equal_temperament(
            frame_interval, num_steps)
    ...
    def generalized_equal_temperament(self, frame_interval=2, num_steps=12):
        ...
        self.pitch_ratios_sharp = [Fraction(k, num_steps)
                                   for k in range(num_steps + 1)]
        self.pitch_ratios_flat = [Fraction(-num_steps + k, num_steps)
                                  for k in range(num_steps)]
        self.pitch_ratios_get = [frame_interval**k
                                 for k in self.pitch_ratios_sharp]
    ...
    def Pythagorean(self, N=12):
        ...
        for k in range(N):
            ...
            while ratios[k] > 2:
                ratios[k] /= 2
        for r in range(N):
            self.pitch_ratios_sharp[(r * 7) % 12] = ratios[r % 12]
        ...
        for k in range(1, N + 1):
            ...
            while ratios[k - 1] < 1:
                ratios[k - 1] *= 2
        for r in range(N):
            self.pitch_ratios_flat[((r + 1) * 5) % N] = ratios[r]
        self.pitch_ratios_pythagorean = self.sort_ratios(
            self.pitch_ratios_flat + self.pitch_ratios_sharp)
    ...
    def sort_ratios(self, ratios):
        ...
        idx = [pylab.argmin(abs(pylab.array(res) - j))
               for j in self.pitch_ratios_eq]
    ...
    def to_dissonance(self, tuning, f0=440., num_harmonics=6):
        ...
        diss = [dissonance_fun(h0 + [p * k for k in harm])
    ...
    def to_scale_sound(self,
        ...
        ramp = pylab.r_[pylab.array(pylab.linspace(0, 1, 100)), pylab.linspace(
            1, 0, num_samples - 100)]
        for rat in tuning:
            x = pylab.r_[x, ramp * harmonics(f0=f0 * rat, **p)]
        return x

    def to_scale_intervals(self,
                           tuning,
                           interval=0,
                           f0=440.,
                           num_harmonics=6,
        ...
        x = self.to_scale_sound(
            dummy, f0=f0, num_harmonics=num_harmonics, dur=dur, sr=sr)
        y = self.to_scale_sound(
            tuning, f0=f0, num_harmonics=num_harmonics, dur=dur, sr=sr)
        return (x + y) / 2
