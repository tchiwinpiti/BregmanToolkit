...
class KMeans(Classifier):
    ...
    def _update_means(self, assignments):
        ...
        if len(empty_classes):
            self.M = self.M[N.setdiff1d(range(self.num_classes),
                                        empty_classes), :]
            old_means = old_means[N.setdiff1d(
                range(self.num_classes), empty_classes), :]
...
class SoftKMeans(KMeans):
    ...
    def _update_means(self, assignments):
        ...
        old_means = self.M.copy()
        for k in range(self.num_classes):
            Xk = N.dot(resp[k, :].reshape(-1, 1), N.ones(
                (1, self.X.shape[1]))) * self.X
...
class GaussianMulti(Classifier):
    ...
    def evaluate(self, data, labels):
        ...
        return len(
            N.where(predicted_labels.reshape(-1, 1) == labels.reshape(-1, 1))[
                0]) / float(len(labels))
    ...
    def evaluate_range(self, data, true_labels, upper_bounds):
        ...
        for i, stop in enumerate(upper_bounds):
            ...
            evaluation[i] = self.labels[N.argmax(
                predicted_counts)] == self.labels[N.argmax(true_counts)]
            start = stop
