...
def play_components(X_hat, F, x=None):
    ...
    for Xh in X_hat:
        ...
        play(c_[x_orig, balance_signal(x_hat)].T,
             F.feature_params['sample_rate'])
...
def ex_1a(x, p, n=4):
    ...
    print("\nExample 1: Standard PLCA with %s" % p['feature'],
          "features split into n=%d" % n, "components")
    F = Features(x, p)
    w, z, h, norm, recon, logprob = PLCA.analyze(F.X, n)
    X_hat = [invert_component(PLCA, w[:, k], z[k], h[k, :])
...
def ex_1b(x, p, n=10, alphaZ=-0.01, alphaH=-0.00001):
    ...
    print(
        "\nExample 1: Standard PLCA with %s" % p['feature'],
        "features split into n=%d" % n,
        "components using Dirichlet priors (alphaZ=%1.6f, alphaH=%1.6f) on component distribution sparseness."
        % (alphaZ, alphaH))
    F = Features(x, p)
    w, z, h, norm, recon, logprob = PLCA.analyze(
        F.X, n, alphaZ=alphaZ, alphaH=alphaH)
    X_hat = [invert_component(PLCA, w[:, k], z[k], h[k, :])
             for k in range(len(z))]
    play_components(X_hat, F, x)
    return w, z, h
...
def ex_2a(x, p, n=4, win=5):
    ...
    #
    print("\nExample 2: Time-shift invariant PLCA with %s" % p['feature'],
          "features split into n=%d" % n,
          "components, component window length=%d" % win)
    F = Features(x, p)
    w, z, h, norm, recon, logprob = SIPLCA.analyze(F.X, n, win=win)
    X_hat = [invert_component(SIPLCA, w[:, :, k], z[k], h[k, :])
             for k in range(len(z))]
    play_components(X_hat, F, x)
    return w, z, h
...
def ex_2b(x, p, n=10, win=5, alphaZ=-0.01, alphaH=-0.00001):
    ...
    print(
        ...
        "features split into n=%d" % n,
        "components, component window length=%d" % win,
        "using Dirichlet priors (alphaZ=%1.6f, alphaH=%1.6f) on component distribution sparseness."
        % (alphaZ, alphaH))
    F = Features(x, p)
    w, z, h, norm, recon, logprob = SIPLCA.analyze(
        F.X, n, win=win, alphaZ=alphaZ, alphaH=alphaH)
    X_hat = [invert_component(SIPLCA, w[:, :, k], z[k], h[k, :])
             for k in range(len(z))]
    play_components(X_hat, F, x)
    return w, z, h
...
def ex_3a(x, p, n=4, win=(1, 5)):
    ...
    print("\nExample 3: Time-frequency-shift invariant PLCA with %s" %
          p['feature'], "features split into n=%d" % n,
          "components, component num shifts=%d,window length=%d" % win)
    F = Features(x, p)
    w, z, h, norm, recon, logprob = SIPLCA2.analyze(F.X, n, win=win)
    X_hat = [invert_component(SIPLCA2, w[:, k, :], z[k], h[k, :, :])
             for k in range(len(z))]
    play_components(X_hat, F, x)
    return w, z, h
...
def separate(clf, x, p, n=5, **kwargs):
    ...
    if clf == SIPLCA2:
        X_hat = [invert_component(clf, w[:, k, :], z[k], h[k, :, :])
                 for k in range(len(z))]
    else:
        X_hat = [invert_component(clf, w[:, :, k], z[k], h[k, :])
                 for k in range(len(z))]
