...
class SparseApproxSpectrum(object):
    ...
    def extract_codes(self, X, standardize=False):
        ...
        self.dico = MiniBatchDictionaryLearning(
            n_components=self.n_components, alpha=1, n_iter=500)
    ...
    def extract_audio_dir_codes(
            self, dir_expr='/home/mkc/exp/FMRI/stimuli/Wav6sRamp/*.wav',
            **kwargs):
        flist = glob.glob(dir_expr)
        self.X = np.vstack([feature_scale(
            LogFrequencySpectrum(
                f, nbpo=24, nhop=1024).X, normalize=1).T for f in flist]).T
    ...
    def reconstruct_spectrum(self, w=None, randomize=False):
        ...
        if self.standardize:
            for comp in components:
                comp = comp * self.std + self.mn
        if randomize:
            components = np.random.permutation(components)
        recon = np.dot(w, components).reshape(-1, self.patch_size[0],
                                              self.patch_size[1])
    ...
    def reconstruct_individual_spectra(self,
        ...
        if plotting:
            ...
            for k in range(self.n_components):
                plt.subplot(self.n_components**0.5, self.n_components**0.5,
                            k + 1)
...
class NMFSpectrum(SparseApproxSpectrum):
    ...
    def extract_codes(self, X, **kwargs):
        ...
        print("NMF...")
        self.model = ProjectedGradientNMF(
            n_components=self.n_components, **kwargs)
    ...
    def reconstruct_spectrum(self, w=None, randomize=False):
        if w is None:
            ...
            w = self.w
        return SparseApproxSpectrum.reconstruct_spectrum(
            self, w=w, randomize=randomize)
