...
def ideal_chromatic_dissonance(num_harmonics=7, f0=440):
    ...
    freqs2 = array(
        [[f0 * 2**(k / 12.) * i for i in harms] for k in range(0, 13)])
...
def audio_chromatic_scale(f0=440, num_harmonics=7):
    ...
    nH = num_harmonics
    H = vstack([harmonics(
        f0=f0 * 2**(k / 12.), num_harmonics=nH, num_points=N)
                for k in arange(13)])
    return H
...
def audio_chromatic_dissonance(f0=440, num_harmonics=7, num_peaks=10):
    ...
    for i, h in enumerate(H):
        F = LinearFrequencySpectrum(
            (h0 + h) / 2., nfft=nfft, wfft=nfft / 2, nhop=nfft / 4)
        P = get_peaks(F)
        frame = []
...
def dissonance_plot(f0=440, num_harmonics=7, num_peaks=10):
    ...
    diss = audio_chromatic_dissonance(
        f0=f0, num_harmonics=num_harmonics, num_peaks=num_peaks)
