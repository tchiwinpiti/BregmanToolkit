...
def extract_audio_chroma(flist, nSecs=10, nSamps=6):
    ...
    for fname in flist:
        ...
        for _ in range(nSamps):
            ...
            chrom = bregman.features.Chromagram(
                y, nfft=8192, wfft=8192, nhop=sr / 20)
...
def load_corpus(corpus=None, idx=None, win_len=1, sample_len=0):
    ...
    corpusNotes = [_extract_notes_positions_and_durations(w)
                   for w in corpusList]
    _report("converting notes to matrices...")
    corpusMtx = [_sample_mtx(_convert_notes_to_matrix(n), win_len, sample_len)
                 for n in corpusNotes]
    _report("done.")
    return corpusMtx
...
def _extract_notes_positions_and_durations(work):
    ...
    """
    notes = np.array([(nn.midi, n.offset, n.quarterLength)
                      for n in work.flat.notes for nn in n.pitches])
...
def plot_mtx(mtx=None, title=None, newfig=False, cbar=True, **kwargs):
    ...
    clip = -100.
    X = scale_mtx(
        mtx, normalize=normalize, dbscale=dbscale, norm=norm, bels=bels)
...
def scale_mtx(M, normalize=False, dbscale=False, norm=False, bels=False):
    ...
    if not (normalize or dbscale or norm or bels):
        ...
        if norm:
            nz_idx = (X * X).sum(1) > 0
            X[nz_idx] = (X[nz_idx].T /
                         np.sqrt((X[nz_idx] * X[nz_idx]).sum(1))).T
        if normalize:
            X = X - np.min(X)
            X = X / np.max(X)
        if dbscale or bels:
