...
class Features(object):
        ...
        if feature == 'stft':
            ...
            if not self._have_cqft:
                ...
                if labels:
                    ...
                    P.yticks(
                        P.arange(0, self._cqtN, self.nbpo),
                        (self.lo * 2**(P.arange(0, self._cqtN, self.nbpo) /
                                       self.nbpo)).round(1))
                    P.xlabel('Time (secs)')
                    P.ylabel('Frequency (Hz)')
        elif feature == 'mfcc':
            ...
            if not self._have_chroma:
                ...
                if labels:
                    self._feature_plot_xticks(
                        float(self.nhop) / float(self.sample_rate))
                    P.yticks(
                        P.arange(0, self.nbpo, self.nbpo / 12.),
    ...
    def _feature_plot_xticks(self, scale):
        ...
        P.plt.xticks(
            x[1:-1], numpy.array(
                x[1:-1] * scale, dtype='float32').round(1))
        P.xlabel('Time (s)')
        P.axis('tight')
    ...
    def _feature_plot_yticks(self, scale):
        ...
        P.plt.yticks(
            y[1:-1], numpy.array(
                y[1:-1] * scale, dtype='float32').round(1))
        P.axis('tight')
    ...
    def _make_log_freq_map(self):
        ...
        logfbws = P.array([max(logfrqs[i] * (f_ratio - 1.0),
    ...
    def _make_dct(self):
        ...
        for i in P.arange(self._dctN):
            for j in P.arange(self._cqtN):
                self.DCT[i, j] = nm * P.cos(i * (2 * j + 1) * (P.pi / 2.0) /
                                            self._cqtN)
        for j in P.arange(self._cqtN):
    ...
    def _overlap_add(self, X, usewin=True, resamp=None):
        ...
        if resamp is None:
            ...
            for k in range(X.shape[0]):
                x[k * nhop:k * nhop + rfft] += sig.resample(X[k],
                                                            rfft) * self.win
        return x
    ...
    def separate(self, cls=plca.PLCA, n=5, play_flag=False, **kwargs):
        ...
        """
        x = kwargs.get('x', None)
        self.w, self.z, self.h, norm, recon, logprob = cls.analyze(self.X, n,
                                                                   **kwargs)
    ...
    def invert_components(self, cls, w, z, h):
        ...
        """
        if cls == plca.SIPLCA2:
            X_hat = [self.invert_component(cls, w[:, k, :], z[k], h[k, :, :])
