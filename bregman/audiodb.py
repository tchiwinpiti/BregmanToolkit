...
class adb:
    ...
    def stack_vectors(data, win=1, hop=1, zero_pad=True):
        ...
        if i / hop < nvecs:
            ...
            features[i / hop, :] = pylab.c_[x, pylab.zeros((1, win * dim -
                                                            x.shape[1]))]
        return features
    ...
    def cov_vector(data,
        ...
        if norm_type == 2:
            data[nz_idx] /= pylab.np.atleast_2d(
                pylab.np.sqrt((data[nz_idx]**2).sum(1))).T
        if norm_type == 3:
            data[nz_idx] /= pylab.np.atleast_2d(abs(data[nz_idx]).max(1)).T
        data = data.reshape(num_blocks, -1, data.shape[1])
        if deltas:
            delta_data = pylab.np.zeros(
                (data.shape[0], data.shape[1], data.shape[2] * 2))
            for i in range(data.shape[0]):
                delta_data[i] = pylab.np.c_[data[i], pylab.np.vstack(
                    [pylab.np.zeros((1, data[i].shape[1])), pylab.np.diff(
                        data[i], axis=0)])]
            data = delta_data
        if mn:
            M = list()
    ...
    def sparseness(data):
        ...
        r = X.shape[0]
        s = (pylab.np.sqrt(r) - pylab.np.abs(X).sum(0) /
             pylab.np.sqrt((X**2).sum(0))) / (pylab.np.sqrt(r) - 1)
        return s
    ...
    def get(dbname, mode="r"):
        ...
        db.configQuery = {'accumulation': 'track',
                          'distance': 'eucNorm',
                          'exhaustive': False,
    ...
    @staticmethod
    def insert_audio_files(fileList,
                           dbName,
                           chroma=True,
                           mfcc=False,
                           cqft=False,
        ...
        for a, i in enumerate(fileList):
            ...
            if chroma: F.feature_params['feature'] = 'chroma'
                ...
                raise error.BregmanError(
                    "One of chroma, mfcc, or cqft must be specified for features")
            F.extract()
    ...
    def insert_feature_files(featureList,
        ...
        for feat, pwr, key in zip(featureList, powerList, keyList):
            ...
            if undo_log10:
                F = 10**F
            if delta_time != 0.0:
                T = pylab.c_[pylab.arange(0, a) * delta_time, (pylab.arange(
                    0, a) + 1) * delta_time].reshape(1, 2 * a).squeeze()
            else:
                T = None
