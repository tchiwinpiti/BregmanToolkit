...
class TimeSpan(object):
    ...
    def __init__(self, start_time=None, end_time=None, duration=None):
        ...
        self.end_time = self.start_time + float(
            duration) if end_time is None else float(end_time)
        if abs(self.end_time - self.start_time - self.duration) > np.finfo(
                np.float32).eps:
            raise ValueError("Inconistent end_time and duration provided")

    def __repr__(self):
        return "start_time=%.3f, end_time=%.3f, duration=%.3f" % (
...
class Segment(object):
    ...
    def __repr__(self):
        return "(label=%s, %s, %s)" % (self.label, self.time_span.__repr__(),
...
class GeneralAudioSegmentor(Segmentor):
    ...
    def extract(self,
                media,
                num_clusters,
                feature=features.LogFrequencyCepstrum,
                filter=True,
                **kwargs):
        """
        ...
        for i in range(len(self.diffs) - 1):
            self.segmentation.append(
                Segment(
                    self.diffs[i] / float(self.frame_rate),
                    self.diffs[i + 1] / float(self.frame_rate),
                    label=seg_labels[i]))
    ...
    def play_segs(self, k):
        ...
        for seg in np.where(clusters == k)[0]:
            ...
            x = sound.wavread(
                self.media,
                first=int(self.segmentation[seg].time_span.start_time * sr),
                last=int(self.segmentation[seg].time_span.duration * sr))
            sound.play(x[0].T, sr)
...
class SegmentationFeatureExtractor(object):
    ...
    def extract(self, segmentation, feature, **kwargs):
        ...
        for seg in segmentation:
            x, xsr, fmt = sound.wavread(
                segmentation.media,
                first=int(seg.time_span.start_time * SR),
                last=int(seg.time_span.duration * SR))
            f = feature(x, **kwargs)
