...
class Evaluator:
    ...
    def __init__(self,
        ...
        if not ground_truth:
            print(
                "You must supply a ground truth as a list of database indices")
        self.adb = None
        ...
        self.set_optional_parameters(seq_length, query_duration, tempo,
                                     tempo_range)
    ...
    def set_optional_parameters(self, seq_length, query_duration, tempo,
                                tempo_range):
        """        
        ::
    ...
    def evaluate_tempo_range(self,
        ...
        for t in tempo_range:
            ...
            ranks, dists = self.evaluate(
                query_duration=query_duration, tempo=t, gt_only=False)
    ...
    def reduce_evaluation_to_gt(self,
                                out_ranks,
                                out_dists,
                                seq_length=None,
                                query_duration=None):
        ...
        gt_keys, gt_lens = self.lower_bound_list_by_length(
            zip(gt_orig_keys, gt_orig_lens), seq_length)
...
class TimbreChannelEvaluator(Evaluator):
        ...
        for gt_item in ground_truth:
            ...
            for t_chan in range(self.timbre_channels):
                ...
                self.initialize_search(
                    t_chan, min([self.seq_length, qlen]),
                    tempo)  # setup per-timbre-channel include list
                t_ikeys.append(self.adb.configQuery[
                    'includeKeys'])  # timbre-channel search keys
            ...
            avg_ranks, avg_dists = self.rank_by_distance_avg(t_qkeys, t_ikeys,
                                                             t_rkeys, t_dists)
            ...
            if res_name is None:
                res_name = tempfile.mktemp(
                    suffix=".dat", prefix="results_", dir=".")
            f = open(res_name, "w")
    ...
    def get_adb_lists(self, tc):
        ...
        lzt = self.adb.liszt()
        lzt_keys, lzt_lens = zip(*lzt)
        return lzt_keys[tc::self.timbre_channels], lzt_lens[
            tc::self.timbre_channels]
